"use strict";
const noop = require("./post-processor").noop;
...
let immutable = (key, url, postProcessor) => {
    postProcessor = postProcessor || noop;
            ...
            if (data) {
                ...
                request(url, function (error, response, body) {
                    if (error) {
                        ...
                        postProcessor(body).then(output => {
...
let mutable = (key, url, postProcessor) => {
    postProcessor = postProcessor || noop;
    var bucket = process.env.AWS_BUCKET_NAME;
    var cachedUrl = getUrl(key);
    console.log(`request: Fetch ${url}.`)
