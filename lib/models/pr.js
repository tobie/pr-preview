...
class PR {
    ...
    getPreviewFiles() {
        if (this.processor == "wattsi") {
            ...
            return this.wattsi.getFilenames().then(files => {
                console.log("this.wattsi.getFilenames(): ", files)
                console.log("this.wattsi.unchangedFiles:", this.wattsi.unchangedFiles);
                this.preview_files = files.map(f => new WattsiFile(this, this.wattsi, f));
                this.unchanged_files = this.wattsi.unchangedFiles.map(f => new UnchangedWattsiFile(this, this.wattsi, f));
            });
        } else {
    ...
    cacheAll() {
        ...
        return next(this.preview_files.slice(0))
            .then(_ => next(this.unchanged_files.slice(0)))
            .then(_ => {
