...
class PR {
    constructor(payload) {
        if (!payload.pull_request) {
            throw new TypeError(`No pull_request property in ${ JSON.tringify(payload, null, 4) }`)
        }
        this._payload = payload;
        this._pr = payload.pull_request;
        this.head = new Head(this._pr.head, this);
        this.base = new Base(this._pr.base, this);
        this.diff = new SpecDiff(this.head, this.base);
    }
    ...
    cacheAll() {
        return Promise.all([this.head.cache(), this.base.cache()])
