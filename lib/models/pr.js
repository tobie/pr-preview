...
class PR {
    ...
    requestMergeBase() {
        return this.request(GH_API.GET_COMPARE_COMMITS, {
            ...
            this.merge_base = MergeBase.fromPR(this);
        });
    }
    ...
    get merge_base_commit() {
        if (!("_merge_base_commit" in this)) {
            throw new Error("Invalid state");
        }
        return this._merge_base_commit;
    }
    
    set merge_base_commit(merge_base_commit) {
        return this._merge_base_commit = merge_base_commit;
    }
    ...
    cacheAll() {
        function next(files) {
            if (files.length) {
                let file = files.pop();
                return file.cache().then(next);
            }
        }
        
        return this.getPreviewFiles().then(next);
    } 
}
