...
let latest = (config, payload, anchors) => {
    ...
    return Promise.all([
        cache(cacheKey(head), bikeshedUrl({
            config:       config,
            ...
            branch:       head.ref,
            sha:          head.sha,
            short_sha:    shortSha(head.sha)
        })),
        cache(cacheKey(base), bikeshedUrl({
            config:       config,
            pull_request: payload,
...
let comment = (config, data, commits) => {
    ...
    return latest(config, payload, anchors).then(txt => {
        return `\n\n\n${ COMMENT_INTRO }\n${ COMMENT_BODY }\n${ anchors.join(", ")}\n${ COMMENT_OUTRO }\n***\n${ txt }`
    });
};
