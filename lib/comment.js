...
module.exports = (payload) => {
    ...
    return getConfig(payload).then(config => {
        return gh.request(GH_API.GET_PR_COMMITS, ghOpts).then(commits => {
            return gh.request(GH_API.GET_PR_FILES, ghOpts).then(files => {
...
let getConfig = payload => {
    ...
    var ghOpts = ghOptions(payload);
    return gh.request(GH_API.GET_CONFIG_FILE, ghOpts).then(file => {
        if (file.type != "file") {
