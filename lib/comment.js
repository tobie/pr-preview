...
module.exports = (payload, force) => {
    ...
    return auth(payload.installation).then(headers => {
        ...
        return getConfig(gh, pull_request).then(config => {
            return gh.request(GH_API.GET_PR_COMMITS, requestOpts).then(commits => {
                return gh.request(GH_API.GET_PR_FILES, requestOpts).then(files => {
                    return updateBody(gh, config, pull_request, commits, files, force).then(release, release);
                });
            });
        });
    });
...
let mutableCacheKey = tip => `${ tip.repo.owner.login }/${ tip.repo.name }/${ tip.ref }.html`;

// owner/repo/branch/7dfd134.html
let immutableCacheKey = tip => `${ tip.repo.owner.login }/${ tip.repo.name }/${ tip.ref }/${ shortSha(tip.sha) }.html`;


// owner/repo/3834774..branch:7dfd134.html
// owner/repo/3834774..contributor:branch:7dfd134.html
let diffCacheKey = (base, head) => {
    let outro = `${ head.label }:${ shortSha(head.sha) }`;
    if (head.repo.owner.login == base.repo.owner.login) {
        outro = `${ head.ref }:${ shortSha(head.sha) }`;
    }
    return `${ base.repo.owner.login }/${ base.repo.name }/${ shortSha(base.sha) }..${ outro }.html`;
}

let bikeshedUrlOptions = (config, pull_request, tip, key) => {
    return {
        config:       config,
...
let latest = (config, pull_request, anchors) => {
    ...
    let head_key = mutableCacheKey(head);
    let base_key = immutableCacheKey(base);
    return Promise.all([
        cache.mutable(head_key, bikeshedUrl(bikeshedUrlOptions(config, pull_request, head, head_key))),
        cache.immutable(base_key, bikeshedUrl(bikeshedUrlOptions(config, pull_request, base, base_key)))
    ]).then(function(urls) {
        let head_url = urls[0];
        let base_url = urls[1];
        let anchor = anchors.length == 1 ? anchors[0] : null;
        return cache.immutable(diffCacheKey(base, head), diff(base_url, head_url, anchor)).then(function(base_diff) {
            let size = Math.floor(40 / anchors.length);
            let preview_url = head_url;
            if (head.repo.owner.login == "whatwg" && base.repo.owner.login == "whatwg") {
                preview_url = `${config.ed_url}/branch-snapshots/${head.ref}/`;
            }
            return `[Preview](${ preview_url }) ${ displayAnchors(preview_url, anchors, size) }| [Diff](${ base_diff })`;
        });
    });
};
...
let body = (config, data, commits, files, force) => {
...
let updateBody = (gh, config, data, commits, files, force) => {
    return body(config, data, commits, files, force).then(b => {
        if (force || b.needsUpdate) {
