...
let latest = (repo, branch, sha, base_branch, base_sha, anchors) => {
    let anchor = anchors.length == 1 ? anchors[0] : null;
    let url = rawgit(repo, sha, anchor);
    let base_url = rawgit(repo, base_sha, anchor);
...
${ COMMENT_OUTRO }
***
${ latest(pr_repo, pr_branch, pr_sha, base_branch, base_sha, anchors) }`
