...
module.exports = (payload) => {
    ...
    let release = b => { currently_running.delete(k); return b; };
...
let getConfig = payload => {
    ...
    return gh.request("GET /repos/:owner/:repo/contents/.pr-preview.json", ghOpts).then(file => {
        ...
        return gh.request(`GET /repos/:owner/:repo/contents/${config.target_file}`, ghOpts).then(file => {
            config.auto_rendering = false; //file.type != "file";
            return config;
        }, _ => {
...
let latest = (config, head, base, anchors) => {
    return Promise.all([
        cache(cacheKey(head), bikeshedUrl(config, head)),
