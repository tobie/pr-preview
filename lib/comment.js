...
let comment = (config, data, commits) => {
    ...
    latest(config, data.head, data.base, anchors).then(latest => {
        return `\n\n\n${ COMMENT_INTRO }\n${ COMMENT_BODY }\n${ anchors.join(", ")}\n${ COMMENT_OUTRO }\n***\n${ latest }`
    });
};
...
let body = (config, data, commits, files) => {
    return new Promise(_ => {
        ...
        if (no_preview || (no_rendered_file && !config.auto_rendering)) {
            return {  updated: false, config: config, needsUpdate: false, previous: body};
        }
...
let updateBody = (config, data, commits, files) => {
    body(config, data, commits, files).then(b => {
        if (b.needsUpdate) {
