...
let perCommit = (pr, current, prev, is_last, anchors) => {
    let anchor = anchors.length == 1 ? anchors[0] : null;
...
let previous = (pr, commits, anchors) => {
    if (commits.length == 1) return "";
    return "<details><summary>Per Commit Preview &amp; Diffs</summary>\n\n" + commits.map((c, i) => {
        return perCommit(pr, c, commits[i-1], commits.length - 1 == i, anchors);
    }).join("\n") + "</details>";
}
...
// ${ previous(data.head, commits, anchors) }
}
