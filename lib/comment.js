...
module.exports = (payload) => {
    ...
    let k = `${payload.base.repo.full_name}/${payload.number}`;
    ...
    var ghOpts = ghOptions(payload);
    return getConfig(payload).then(config => {
        return gh.request("GET /repos/:owner/:repo/pulls/:number/commits", ghOpts).then(commits => {
            return gh.request("GET /repos/:owner/:repo/pulls/:number/files", ghOpts).then(files => {
                return updateBody(config, payload, commits, files).then(release, release);
            });
        });
    });
};
...
let getConfig = payload => {
    ...
    var ghOpts = ghOptions(payload);
    return gh.request("GET /repos/:owner/:repo/contents/.pr-preview.json", ghOpts).then(file => {
        if (file.type != "file") {
        ...
        return gh.request(`GET /repos/:owner/:repo/contents/${config.target_file}`, ghOpts).then(file => {
...
let bikeshedUrlOptions = (config, payload, tip) => {
    return {
        config:       config,
...
let latest = (config, payload, anchors) => {
    let head = payload.head;
    let base = payload.base;
    return Promise.all([
        cache(cacheKey(head), bikeshedUrl(bikeshedUrlOptions(config, payload, head))),
        cache(cacheKey(base), bikeshedUrl(bikeshedUrlOptions(config, payload, base)))
    ]).then(function(urls) {
        let head_url = urls[0];
        let base_url = urls[1];
        let anchor = anchors.length == 1 ? anchors[0] : null;
        return Promise.all([
            cache(diffCacheKey(base, head, true),  diff(config.ed_url, head_url, anchor)),
