...
module.exports = (payload) => {
    ...
    return gh.request("GET /repos/:owner/:repo/pulls/:number/commits", options).then(commits => {
        return gh.request("GET /repos/:owner/:repo/pulls/:number/files", options).then(files => {
            return updateBody(payload, commits, files);
        });
    });
};
...
let diffWithED = (base, current, anchor) => diff(edURL(base), current, anchor);
...
let latest = (pr, base, anchors) => {
    ...
    let ed_diff = diffWithED(base, url, anchor);
...
let perCommit = (pr, base, current, prev, is_last, anchors) => {
    let anchor = anchors.length == 1 ? anchors[0] : null;
    let url = rawgit(pr.repo.full_name, current.sha, anchor);
    let diffWED = diffWithED(base, url, anchor);
    let prevTxt = "";
...
let previous = (pr, base, commits, anchors) => {
    if (commits.length == 1) return "";
    return "<details><summary>Per Commit Preview &amp; Diffs</summary>\n\n" + commits.map((c, i) => {
        return perCommit(pr, base, c, commits[i-1], commits.length - 1 == i, anchors);
    }).join("\n") + "</details>";
}
...
${ COMMENT_OUTRO }
***
${ latest(data.head, data.base, anchors) }`
// ${ previous(data.head, data.base, commits, anchors) }
}

let body = (data, commits, files) => {
...
let updateBody = (data, commits, files) => {
    let b = body(data, commits, files);
    b.updated = false;
    if (b.needsUpdate) {
