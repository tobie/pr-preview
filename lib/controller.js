...
class Controller {
    ...
    setCache(params, pr) {
        ...
        let pr_url = `https://github.com/${pr.owner}/${pr.repo}/new/${pr.base.branch}?filename=${f}&value=${c}`;
        this.previewer_cache.set(this.cacheKey(params), pr_url);
    }
    ...
    getCache(params) {
        this.previewer_cache.get(this.cacheKey(params));
    }
    ...
    pullRequestUrl(params) {
        ...
        return url ? Promise.resolve(url) : this.getUrl(params);
    }
    ...
    needsUpdate(pr, result) {
        return pr.touchesSrcFile() && (result.forcedUpdate || pr.requiresPreview());
    }
