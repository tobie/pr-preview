...
class Controller {
    ...
    queueJob(job) {
        ...
        if (isDuplicateInQueue || isCurrentlyRunning) {
            job.skipped = true;
            job.skipReason = isCurrentlyRunning ? 'already processing' : 'already queued';
            return job;
        }
    ...
    async processQueue(callback, errorCallback) {
        while (this.queue.length > 0) {
            ...
            this.currently_running.add(result.id);

            try {
                result = await this.handlePullRequest(result);
                callback(result);
            } catch (error) {
                errorCallback(error);
            }
