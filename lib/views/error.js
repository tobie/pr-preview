...
`;
    ...
    service(err) {
        ...
        return service ? `PR Preview relies on a number of web services to run. There seems to be an issue with the following one:

        :rotating_light: [${service.name}](${service.url})` : "";
    }
...
_If you don't have enough information above to solve the error by yourself (or to understand which web service is causing the issue), please [file an issue](${ this.issueUrl(pr) })._
</details>`;
    }
